...
## История изменений
- **2026-06-20** — деплой: **авто-перезагрузка при устаревшем чанке** (gh-pages `b65671b5`,
  main `e2eec0f6`, bundle `index-jI_c0uaU`). Причина: у диспетчера «не грузилась» сторона —
  закэшированный старый каркас ссылался на удалённые имена чанков (404 при lazy-import).
  Бэкенд и деплой были исправны (все 33 чанка 200) — проблема чисто клиентского кэша.
  Фикс в `src/main.tsx`: слушаем `vite:preloadError` + import-reject → 1 перезагрузка (счётчик
  от цикла). APK v2 перекопирован (9718188 б). Немедленный обходной для застрявших: полностью
  закрыть и открыть приложение (autoUpdate SW подхватит новый index).
